...
class KtorRelayTransport(
     ...
     *      [SyntheticTriggerResult.RefusedAlreadyFired]. The fresh
     *      allowance on each new Connected emission is implicit —
     *      [wsSessionEpoch] advances per Connected so a stale "last
     *      consumed epoch" naturally fails to equal the current one
     *      (production does NOT reset the latch; PR #353 round 3).
     *
     ...
     * is silently absorbed. The dispatcher's sequential
     * single-consumer-coroutine guarantee ensures state visibility
     ...
     * tests. Force-sets [_state] to the supplied value WITHOUT going
     * through the real WS handshake / disconnect path. Pairs with
     * [bumpSessionEpochForTest] which advances [wsSessionEpoch] without
     * going through the real reconnect loop.
     *
     * Name matches the `*ForTest*` deny pattern; the
     * `verifyR8StripsTestSeams` Gradle task fails the release build if
     * this method survives R8.
     */
     ...
     * unit tests. Resets [oneShotLatchConsumedAtEpoch] to `null`.
     *
     ...
     * claims that had already taken the mutex. Tests that want to
     * simulate a "fresh epoch with old latch state" use
     * [bumpSessionEpochForTest] instead; this method is kept for the
     * narrow case of asserting the pre-reset latch behaviour.
     */
