...
**Last updated:** 2026-05-21 (thu) — Voice recorder UX hardening: `52a9773f` (#197 PR-UI-REC2 Locked + hold-to-lock + REC2.1 routing + REC2.2 lifecycle + REC2.3 ACTION_DOWN state machine + REC2.4 Locked-send routing + swipe-left-cancel + recorder crash hardening) and `b117dcb9` (#198 PR-MEDIA-UPLOAD-CANCEL1 + CANCEL2 + CANCEL2.1). 9 real-device test rounds (#76 → #76.5b) drove the iterations; both PRs PASS the final acceptance matrix. SwipeCancel-state visual deferred to PR-UI-REC3. Open follow-ups: PR-UI-REC3, recording-duration source (durationMs ticker undercounts vs MediaMetadataRetriever), empty-voice race when heldMs > 700ms but durationMs=0, re-enable M2e early manifest after relay grows receiver-side media-cancel protocol, receiver-side download-cancel needs relay endpoints, notifications-flakiness diag PR.  
