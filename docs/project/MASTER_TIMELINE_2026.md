...
| 4 | `fix/strict-fifo-outbox-pr-h2a` | **PR-H2a:** strict FIFO outbox via per-envelope monotonic `sequenceTs` + `outboundSendMutex` serializing live-send vs flush. Wire format unchanged. 5 unit tests including Test #33 layout. PR-H2b (skip-message-keys + bounded retry) is the follow-up | 🟡 open, awaits manual Test #33 retest |
...
**Deferred follow-ups:**
- **PR-H2b (next after H2a merges):** skipped-message-keys in the ratchet (per Signal Spec) + bounded retry on MAC error + dead-letter log. PR-H2a closed the *client-side* reorder source (outbox FIFO); H2b makes the receiver tolerant of any *network-layer* reorder (TCP retransmit pause, multi-path mobile, Tor circuit shifts). Without H2b, any future scenario that re-orders frames between sender's wire and receiver's decrypt will silently drop messages exactly like Test #33 did.
- WS pong timeout cycle every 60–110 s on both devices remains as **PR-H1b/c** material (separate from H2 line). Auto-recovery works after H2a merges, so messages arrive but the UI shows momentary "lagging."
