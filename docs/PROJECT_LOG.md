...
- **`docs/tracks/rc-direct-stability1.md` Arm C Outcome subsection added** under §4 Arm C — matrix table, mechanism explanation (writePingFrame fails on next ping after awaitingPong unanswered → 3× implies ~2 missed Pongs before detection), Mode 2 signature persistence, ship-criterion FAIL verdict, deceptive-metric warning, memory pointer.
