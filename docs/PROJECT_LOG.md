...
- **✅ PR-D2a — Voice / call UI + send-layer guard for Limited realtime.**
  ...
  end-to-end on Tecno `<redacted-device-serial>` (Tele2 LTE Иркутская) across
...
**Outcome.** All three tracks landed cleanly. Master head advances `fe14c977 → bbf0cfff` via PR #364; PR #330 closed as superseded (no code delta); B2 mini-lock ships as docs-only on this PR head.

**Track C — BodyTimeoutContractTest unquarantine (PR #364 squash `bbf0cfff`).** Two-phase diagnostic before landing the fix:

- **Diagnostic phase 1 (PR #362, closed without merge).** Four CI Ubuntu runs on a probe branch (`diagnostic/body-timeout-hang-probe`) established via successively-closer-to-verbatim jvmTest probes (`v1` stripped; `v2` verbatim time-advance patterns + no `withTimeout` mask; `v3` no-DebugProbes control; `v4` verbatim `BodyTimeoutContractTest` fixture duplicate in `jvmTest`) that all four went CI green — meaning the hang is source-set sensitive (`commonTest` vs `jvmTest`). Then Option D (removed `@kotlin.test.Ignore` on the original `commonTest` class only, ZERO other change, head `d2ba8056`) triggered the 30-minute job timeout `08:31:16Z → 09:01:30Z` verbatim per the KDoc's historical Round 3-5 pattern, confirming the hang STILL fires on current master `fe14c977` post-MC-stack.
- **Diagnostic phase 2 (PR #363, closed without merge).** Option E `expect`/`actual` `HangDiagnostic` seam (`commonTest/HangDiagnostic.kt` + `jvmTest/HangDiagnostic.jvm.kt` (DebugProbes + wall-clock daemon watchdog) + `androidUnitTest/HangDiagnostic.android.kt` (no-op)) instrumented the un-quarantined `BodyTimeoutContractTest` cells on the diagnostic branch. CI head `5ff972f9` cancelled at the 30-minute timeout on `r12_body_timeout_after_headers_accounts_toward_breaker`, but with 54 watchdog fires captured 60-second stack + `DebugProbes.dumpCoroutines` snapshots to stderr. Findings: `coroutine#113` (`pollJob`) caught state `RUNNING` at `pollLoop:1568/1570` on every fire; `_breakerTimerJob` coroutines regenerated at ~6,400 launches / second real time (~11.5 million over the 30-min cancelled job); test worker thread `RUNNABLE` inside `TestCoroutineScheduler.advanceUntilIdleOr`. Dump archived durable at `C:/temp/body-timeout-hang-dump-2026-07-03/` (`pr363-full-ci.log` + `SUMMARY.md`).
- **Path 3 code read (static analysis on the dump).** `pollLoop` / `wsActivePollLoop` / `aliveTickLoop` all read `while (scope.isActive)` where `scope = CoroutineScope(SupervisorJob() + dispatcher)`. The `SupervisorJob()` has no parent, so `job.cancel()` on the individual poll job cancels the child only; the SupervisorJob itself stays `Active`. `scope.isActive` therefore stays `true` even when the current coroutine has been cancelled — the loops rely solely on `delay(...)` throwing `CancellationException` as their sole cancellation-exit path, which is a stale invariant regardless of the CI hang.
- **Path X production fix (PR #364 squash `bbf0cfff`).** Three-line change in `shared/core/transport/src/commonMain/kotlin/phantom/core/transport/RestFallbackOrchestrator.kt`: `while (scope.isActive)` → `while (currentCoroutineContext().isActive)` in `pollLoop:1539` + `aliveTickLoop:1854` + `wsActivePollLoop:1897`; add `import kotlinx.coroutines.currentCoroutineContext`. Extended comments at each site preserve the orphan-scope-Job explanation so a future reader does not revert to `scope.isActive` on cosmetic grounds. Class-level `@kotlin.test.Ignore` removed from `BodyTimeoutContractTest`; the historical Round 3-5 quarantine KDoc block preserved verbatim as evidence; a new "UNQUARANTINE ATTEMPT (2026-07-04) — Path X fix candidate" block appended below documenting the dump + root-cause identification. Local verification: `:shared:core:transport:jvmTest --tests BodyTimeoutContractTest` 6 tests / 0 failures / 0 errors / 0.29 s; full `:shared:core:transport:jvmTest` 622 tests / 0 failures / 0 errors / 18 s. CI Ubuntu 4/4 SUCCESS on head `bd532934`. Merged. PR #362 + PR #363 closed without merge; diagnostic seam + `kotlinx-coroutines-debug` dep did NOT make it to master.

**Track A — PR #330 close-out (superseded).** RC-RECONNECT-QUIESCENCE1 PR #330 (`canary/reconnect-quiescence` head `6f49cd89`, Draft since 2026-06-23) closed as superseded 2026-07-04. Byte-identity check vs current master `bbf0cfff`: `shared/core/transport/.../WsReconnectGate.kt` 0 lines diff, `shared/core/transport/.../WsReconnectGateTest.kt` 0 lines diff, `apps/android/.../TransportRewalkCoordinator.kt` 0 lines diff. `git diff master origin/canary/reconnect-quiescence --stat` reports 233 "canary-side" characters as effectively (i) older comment provenance labels (e.g. "commit 2e fix-round-1 (2026-06-22)" that master's squash-merge rewrote to reference MC-1/2/3 provenance) and (ii) canary being behind master on subsequent work (PR #353 L1 mini-lock, Sprint 2b-A/B/C, PR #364). No functional code unique to canary. Closing comment fixation includes: MC-1 (PR #358 `91745acd`) + MC-2 (PR #359 `7bcf2d04`) + MC-3 (PR #360 `fd435c93`) + B1 closure (PR #361 `fe14c977`) + PR #364 `bbf0cfff` map where each part of PR #330's original scope landed; **explicit LTE rollout gate note** (Wi-Fi B1 PASS does NOT unblock LTE per parent recon §11 amendment; RC-RECONNECT-QUIESCENCE1 code shipping to LTE users still requires either (i) B2 track closes with a fix that resolves the Tele2 LTE long-connection surface, or (ii) an alternative transport lands separately — neither path is on this branch's remit); **release-flag posture unchanged** (`RECONNECT_QUIESCENCE_ENABLED`, `MODE_2_FAST_PATH_ENABLED`, `MODE_2_STICKY_ENABLED` all stay literal `"0"` in the release block; production rollout to Wi-Fi users happens through a separate flag-flip PR, not by this closure).

**Track B — DIRECT-WSS-MODE2-B2-LTE-RECON1 mini-lock (this PR, docs-only).** Materialises the B2 forward-pointer from `direct-wss-mode2-recon1.md` §11 (2026-06-28 amendment). Track-doc at `docs/tracks/direct-wss-mode2-b2-lte-recon1.md` (10 sections). Scope: Tele2 LTE real-device delivery reliability. Hypotheses split into two orthogonal levels — **causal** (H-B2-Carrier-NAT-Idle / H-B2-Byte-Freeze / H-B2-Age-Kill / H-B2-Doze-OEM / H-B2-Config-Mismatch) and **fault-scope** (H-B2-REST-Survives-WSS-Fails / H-B2-REST-LongPoll-Separate-Fault / H-B2-Both-Fail). Seven diagnostic instruments B2-K1 through B2-K7 explicitly NOT pre-committed beyond the "B2-K1 + B2-K4 first" §7 requirement. Six acceptance gates PASS-A (idle-time threshold + keep-alive fix candidate) / PASS-B (byte-or-age threshold + rotation candidate) / PASS-C (Doze / OEM cause + Android-side mitigation candidate) / PASS-D (REST survives while WSS fails + faster fall-back strategy candidate) / FAIL-Wider-Issue (both fail; requires alternative transport OR relay hold-policy change) / PARK (2 clean B2-K1 attempts with no reproduction, redirects to next-network-class candidacy). First deliverable = **B2-K1 + B2-K4** on one Tele2 LTE real-device session after mini-lock merges. Operator-run smoke template is a follow-on artefact NOT shipped by this PR. Facts-first per WORKING_RULES rule 3.

**In-force lineage.** Methodology recon (PR #349 `54f2e50d`) stays Closed with verdict H-ME. `direct-wss-mode2-recon1.md` §11 amendment + §13 B1 CLOSED verdict in force. `TELE2-LTE-REST-BREAKER-RECON1` §11 closure carry-forward. `rc-direct-stability1.md` § 13 (Direct WSS Mode 2 root-cause park at Arm G v10/v11 write-stall) carry-forward. L1 mini-lock (PR #353 `ed3406eb`) + MC-1 (PR #358 `91745acd`) + MC-2 (PR #359 `7bcf2d04`) + MC-3 (PR #360 `fd435c93`) + B1 closure (PR #361 `fe14c977`) + Path X fix (PR #364 `bbf0cfff`) all in force.

**Non-blocking follow-ups (opened separately, not by this PR):** smoke script delivery invariant fix (script `smoke-run.sh` currently only counts `REST_TRACE inbound_deliver`, missed WS-path deliveries in B1 run 2 and produced false-positive `FAIL`) / §4.3 P3 regex hardening deferred from PR #360 Round 4 revert. Both remain queued at operator's convenience; neither blocks B2 execution.
...
**Fix-candidate APK.** Built from master `fd435c93` (MC-3 merged head) with four flags forced to `"1"` via `-P` gradle properties on the release-flavor `assembleRelease` task: `RECONNECT_QUIESCENCE_ENABLED` + `MODE_2_FAST_PATH_ENABLED` + `MODE_2_STICKY_ENABLED` + `DEBUG_FORCE_MODE_2_DETECTION`. Sideloaded to Tecno BF7-12 (`adb serial <redacted-device-serial>`) + emu pair. APK size 202 MB, `sha256: 5d3de3790b999f957e667d63dcf93539ecbb0015867de7ef3703b2161ade304e`. Build was the docs author's responsibility, not the operator's.
...
**Why this matters:** §13.1's MUST NOT bullet 3 explicitly forbids "PR #330's full RestStateMachine / KtorRelayTransport / TransportRewalkCoordinator modifications" in the carve-out — "Only the gate component compiles + unit-tests; the larger production-behaviour rewrites stay on PR #330's branch." Bringing `WsReconnectGateTest.kt` verbatim would force importing those `RestStateMachine` modifications, putting the §13.1 MUST IN and MUST NOT bullets in direct conflict. The original wording ("at minimum `WsReconnectGateTest.kt` (PR #330's 1172 LOC) brought across or re-authored") reflected an unverified assumption that the file was gate-in-isolation; the facts-first recon refuted that assumption.

**Three paths considered:**

- **Path A** — open gate-only carve-out without `WsReconnectGateTest.kt`; write fresh structural tests; explain in PR body. Rejected because §13.1 already on master explicitly names the file as MUST IN; reviewer would correctly catch a §13.1 violation in the carve-out PR.
- **Path B** — bring `WsReconnectGateTest.kt` verbatim AND the `RestStateMachine` changes needed to compile it. Rejected because it violates §13.1 MUST NOT bullet 3 and breaks the whole gate-only-carve-out intent.
- **Path C** — small docs-only amendment now correcting §13.1's test scope BEFORE opening the carve-out; carve-out then opens against the corrected wording. CHOSEN.

**What this PR ships:**

- **`docs/tracks/quiescence-validation-mc-half-mini-lock.md`** — §13.1 "Targeted unit tests" bullet replaced with a "Fresh structural unit tests for the gate types" bullet that lists the gate file's actual surface to cover (sealed-class distinctness, `ProbeToken.toString` redaction, `CandidateProving` type-level absence of `token`, `simpleKind()` coverage, `ProbeBudget` constant pins, sealed-hierarchy distinctness for `WsReconnectPermit / ClaimResult / ProbeIssueResult / RouteChangeOutcome`, enum exhaustiveness for `ClaimFailureReason / ProbeIssueRejectReason`). §13.1 MUST NOT extended with explicit "defer to MC PR" entries for both `WsReconnectGateTest.kt` and `RestFallbackOrchestratorQuiescenceWiringTest.kt`. §13.4 "Do NOT" line extended to forbid bringing either integration test into the carve-out. New §13.5 ("Test scope correction — recon finding") records the discovery + cites the line numbers in `WsReconnectGateTest.kt` that depend on `RestStateMachine`. §1 status header line updated.
- **`docs/PROJECT_LOG.md`** — this entry.
- **`docs/project/MASTER_TIMELINE_2026.md`** — new-PR pointer entry.

**What this PR does NOT change:**

- §1-§12 of the mini-lock stay in force.
- §4 binding constraints (L-13.3.1 / .3 / .4 / .5 / .11) remain binding on the MC implementation PR.
- §6 load-bearing hypotheses remain binding on the MC implementation PR.
- §7 acceptance-gate criteria for MC PASS / MC PARTIAL / MC FAIL remain unchanged.
- §13.2 sequencing — six-step contract gate-carve-out → MC implementation PR → Wi-Fi smoke → B1 closure unchanged.
- §13.3 carry-forward statement unchanged.
- §13.4 hand-off contract unchanged at the structural level (only the "Do NOT" line gains the integration-test-deferral bullet).
- Strategy 1 lock from the §13 amendment proper (PR #355 squash `ca12a98c`) — unchanged.
- PR #330's contract. PR #353 (MB half) — unchanged. The methodology recon's H-ME verdict — unchanged.
- L1 mini-lock + L1 §5.1 Option A amendment + path-2 step 2 narrowing PR — unchanged.

**Follow-up:** After this amendment merges on master, the gate-only carve-out PR opens against §13.1's corrected test-scope wording. It brings `WsReconnectGate.kt` (verbatim or re-authored) + a fresh structural unit-test file (~100-150 LOC) + `RECONNECT_QUIESCENCE_ENABLED="0"` BuildConfig flag + companion release-pin test + narrowed ProGuard discipline extension (only if new `KtorRelayTransport` members are surfaced — per the gate file content as it stands, none are). Wiring stubs (constructor parameters, AppContainer-side wire-up) are NOT mandatory and the expected default is "no wiring stubs" — extracting `WsReconnectGate.kt` imposes no compile-time dependency on any existing class. If wiring is introduced anyway, the carve-out PR body MUST cite the specific compile-time dependency that forced it. No `RestStateMachine` modifications either way. The `WsReconnectGateTest.kt` (1172 LOC) and `RestFallbackOrchestratorQuiescenceWiringTest.kt` lift into the MC implementation PR alongside the gate's state-transition implementation on `RestStateMachine`.

**Key commits:** TBD (this PR's squash SHA backfilled in MASTER_TIMELINE on merge).
...
- **§13.2 Sequencing** — six steps from carve-out PR open through B1 closure, consistent with the §9 / §12 "Wi-Fi smoke is the final field-shape check before B1 closes" contract: (1) gate-only carve-out PR opens / (2) merges on master, becomes qualifying base for MC implementation PR / (3) MC implementation PR opens stacked on landed gate / (4) MC PASS + already-landed MB PASS satisfies the two H-ME validation halves and UNLOCKS the controlled Wi-Fi smoke run — does NOT close B1 on its own / (5) Wi-Fi smoke run opens as separate operator-scheduled item; Wi-Fi smoke PASS is what closes B1 per `direct-wss-mode2-recon1.md` §11 / (6) PR #330 advances per its own mini-lock contract after B1 closes (i.e., after MC PASS + MB PASS unlocked the smoke AND the smoke itself PASSed). Carve-out reduces PR #330's effective diff (gate already on master), simplifying eventual integration.
- **§13.3 What this amendment does NOT change** — §1-§12 stay in force; Strategy 2/3 NOT carried forward; §4 binding constraints remain binding on MC implementation PR; §6 hypotheses remain binding; §7 acceptance-gate criteria for MC PASS / PARTIAL / FAIL unchanged (the §7.1 MC PASS disposition *wording* aligned to the §13.2 sequencing — MC + MB unlocks Wi-Fi smoke; B1 closes only after smoke PASS — matching the §9 / §12 contract already in force); PR #330's contract NOT amended; PR #353 NOT amended; H-ME verdict NOT amended; L1 mini-lock + L1 §5.1 Option A amendment + path-2 step 2 narrowing PR all stay in force.
- **§13.4 Hand-off to the gate-only carve-out PR** — opens AFTER this amendment merges; cites this amendment SHA + PR #330's gate-source provenance; states carve-out scope per §13.1 explicitly; documents BuildConfig flag wiring + release pin; includes companion release-pin test; includes narrowed ProGuard extension if needed; states explicitly that carve-out is structural — no observable production transport behaviour change when flag is `"0"` (bytecode itself differs due to new class files + BuildConfig field, but no production code path observes a different runtime behaviour); states that PR #330 contract NOT amended; that MB half NOT amended; that L1 mini-lock + path-2 step 2 narrowing all stay in force. After carve-out merges, MC implementation PR's preconditions per §8 are satisfied. Do NOT open carve-out with flag pinned `"1"`. Do NOT skip companion release-pin test. Do NOT touch production runtime path when flag is `"0"`. Do NOT pre-empt MC implementation PR scope.
...
- After MC PASS + the already-landed MB PASS: the Wi-Fi smoke run mini-lock opens (the smoke is the final field-shape check per §9 / §12 of the MC half mini-lock). B1 closes after Wi-Fi smoke PASS, NOT after MC + MB alone.
...
**§7 acceptance gates** — three closure dispositions (MC PASS / MC PARTIAL / MC FAIL). MC PASS combined with the already-landed MB PASS satisfies the two H-ME validation halves and unlocks the controlled Wi-Fi smoke run as a separate operator-scheduled item; B1 closes only after Wi-Fi smoke PASS. MC PARTIAL surfaces specific failures and leaves PR #330 Draft / HOLD; MC FAIL means PR #330's gate design has a model-level defect and the next step is operator-led re-design.
...
The mini-lock binds the implementation PR to all eleven locks `L-13.3.1` through `L-13.3.11` from the methodology recon's §13.3 closure verdict, split across §4 + §5 + §7 (§4 carries the eight non-blocker / non-race locks L-13.3.1 / .2 / .3 / .4 / .5 / .8 / .10 / .11; §5 carries the two BLOCKER locks L-13.3.6 / L-13.3.7 as preconditions that the implementation PR's initial diff MUST already satisfy — Option A "stack on already-landed PR #330 narrowing commit cited verbatim" or Option B "include ProGuard narrowing + R8 verification in the same initial implementation PR diff, before or alongside any synthetic-trigger code"; §5.2 four-layer protected operator surface with four-layer mapping required for any operator-trigger shape, raw `am broadcast` BroadcastReceiver without explicit four-layer mapping forbidden; §7 carries the L-13.3.9 race-contract lock and pins three implementation-PR-binding resolution requirements but does NOT resolve the race itself — single epoch snapshot at top of method, receiving-side dedup mechanism chosen from D-1 / D-2 / D-3 (rely on RestStateMachine epoch filter / dispatcher tuple dedup / method-level mutex), adversarial race-window tests that force a real session death between guard and `trySend`); formalises the `SyntheticTriggerResult` typed-return contract concretely as a sealed class with five required members `Fired / RefusedDisabled / RefusedNotConnected / RefusedDurationOutOfRange / RefusedAlreadyFired` plus the L-13.3.5-derived optional sixth `RefusedNoRestCapability` (§6); binds the implementation PR's test set to the source-grounded acceptance matrix at `C:\temp\quiescence-h-me-council-2026-06-29\tests.md` as test floor not ceiling (§8); specifies the implementation PR's hand-off contract — when it may open, what it must contain, what verdict it must produce (§9); records three park conditions (P-1 PR #330's narrowing plan changes / P-2 operator unavailable / P-3 architectural change in `KtorRelayTransport` or dispatcher); enumerates what is NOT pre-decided so the implementation PR retains design freedom on file naming, signature permission string, exact component name, exact dedup option, Gradle task implementation.
...
**Mode 2 detection chain on master HEAD (the destination a synthetic trigger would feed):** two downstream consumers of a single upstream lifecycle event, dispatched SEQUENTIALLY in `WsSessionLifecycleDispatcher.dispatch(Ended)` — state-machine consumer first (line 55 `submitStateEvent`), telemetry consumer second (line 59 `feedDegradationDetector`). Shared upstream: `KtorRelayTransport` per-session `finally` block parses close-error via `PingTimeoutTextParser`, computes `okhttpPingTimeoutDetected`, emits `WsSessionLifecycleEvent.Ended(...)` into `_wsSessionLifecycle: Channel<WsSessionLifecycleEvent>` (line 1029-1040). Consumer A (state-machine actuation, R3.6): dispatcher line 55 → `submitStateEvent(toRestStateMachineEvent()) → RestStateMachine.onWsSessionEnded` → 3-condition signature check (`inboundFrames == 0 && okhttpPingTimeoutDetected && durationMs in MODE_2_MIN..MAX`) → if `mode2FastPathEnabled = true` → `transitionToRest("mode_2_fast_path")` + if `mode2StickyEnabled = true` → `armSticky()` → emits `REST_TRACE sticky_armed gen=N reason=mode_2_fast_path`. Consumer B (telemetry-only on master): dispatcher line 59 → `feedDegradationDetectorOnWsSessionEnded → WsDegradationDetector.recordAndEmit(Event.PingTimeout, kind)` → `WS_DEGRADED_TELEMETRY` log lines (no action per the detector class KDoc line 59: "No `markSuspect` action. This detector only logs."). PR #330's quiescence chain hooks on top of `armSticky()`. Both `mode2FastPathEnabled` and `mode2StickyEnabled` are `Boolean` constructor parameters on `RestStateMachine` (commonMain); Android-side `AppContainer.kt:1348-1357` reads `phantom.android.BuildConfig.MODE_2_FAST_PATH_ENABLED == "1"` / `MODE_2_STICKY_ENABLED == "1"` and passes the Booleans down — commonMain code never reads `BuildConfig` directly.
...
**Sketch of §12.4 L1 surface** (NOT a proposal — §5 design-exercise output only) — must respect commonMain / Android isolation rule (shared code cannot see `phantom.android.BuildConfig`; established pattern at `AppContainer.kt:1348-1357` reads on Android side and injects Boolean to shared constructor): one new Android-side BuildConfig flag `DEBUG_FORCE_MODE_2_DETECTION` defaulting `"0"` in release block (mirroring existing `MODE_2_FAST_PATH_ENABLED` / `MODE_2_STICKY_ENABLED` / `RECONNECT_QUIESCENCE_ENABLED` pattern with companion `Mode2DebugForceReleaseBuildConfigPinTest` in `androidUnitTest`); one new constructor parameter on `KtorRelayTransport` — `debugForceMode2Enabled: Boolean = false` (or `debugForceMode2Provider: () -> Boolean = { false }` if lazy re-evaluation needed). AppContainer reads the BuildConfig literal on the Android side and injects the resulting Boolean / provider down. One new `internal fun debugForceMode2Synthetic(durationMs: Long): Boolean` method on `KtorRelayTransport` (`internal` only, no `public` alias), gated at the top by the injected Boolean/provider; refuses if `_state.value !is TransportState.Connected`; refuses if `durationMs` lies outside `MODE_2_MIN_DURATION_MS..MODE_2_MAX_DURATION_MS`; builds the synthetic event satisfying the signature check (`inboundFrames = 0, okhttpPingTimeoutDetected = true, closeOrigin = "synthetic", closeError = "DEBUG_FORCE_MODE_2_DETECTION", pendingAcksAtClose = 0, sessionEpoch = current epoch`); calls `_wsSessionLifecycle.trySend(WsSessionLifecycleEvent.Ended(...))` and returns the success Boolean; one operator-facing trigger surface on the Android side (debug menu OR `am broadcast` intent handler registered in AndroidManifest debug block — design left open at sketch level).
...
**New verdict class — B1 non-regression PASS / quiescence NOT EXERCISED.** §11 anticipated B1-PASS / B1-PARTIAL / B1-FAIL but not "all six hypotheses meet the letter while the spirit (mechanism actually tested) was not met". K-6 hypothesis matrix: 1 PASS (H-330-No-Message-Loss-Or-Dups), 4 NOT EXERCISED (H-330-Quiesces-Storm, H-330-Single-Probe-Per-RouteChange, H-330-Probe-Lives-60s in quiescence context, H-330-No-Self-Reentry), 1 VACUOUSLY PASS (H-330-Preserves-REST — present REST events but not stress-tested by a quiescence window). Mode 2 detector did not fire on Wi-Fi (consistent with K-1's F-Mode1 carry-forward; the detector targets the Mode 2 signature specifically per PR #328's R3.6 design). Five `mode_switched ... reason=ws_alive_60s` events observed (Tecno 4 + emu 1) but each was a R3.6 sticky-per-route `WsCandidate → WS_ACTIVE` transition WITHOUT preceding `sticky_armed` — these are NOT quiescence-recovery probes despite the matching log field name.

**What K-6 DID establish:** PR #330 with three flags `"1"` does not regress Wi-Fi delivery; messages flowed in both directions; zero `relay_send_return ok=false`; reconnect ratios (Tecno 33/16 = 2.06×, emu 33/16 = 2.06×) unchanged vs K-1 baseline (2.0× both); Sprint 2b-C inbound_repair_ok promotion=true works on Wi-Fi (7 `pending_fallback_fail` per side all followed by successful `inbound_repair_ok promotion=true`, zero `DECRYPT_TRACE fail_mac`, zero `OpkNotFound`, zero `inbound_repair_fail`, zero `action=hold` — positive data point against the K-2 LTE side-finding's "Sprint 2b-C might regress" worry).

**What K-6 did NOT establish:** core PR #330 quiescence chain (`sticky → probe → ws_alive_60s recovery`) NOT field-validated. Four NOT-EXERCISED hypotheses cover the load-bearing claims of PR #330's mini-lock. PR #330 is NOT closer to ship-readiness on the strength of K-6 alone. The §11 release / rollout gate stays in force.

**Phase B status after K-6:**

- B1 acceptance gate: NOT closed. K-6 meets §11's letter (no hypothesis FAILED) but not its spirit (mechanism actually tested). A future deliverable that exercises the quiescence chain is required.
- K-3 (emu Wi-Fi-only) remains NOT promoted.
- PR #330 stays Draft / HOLD; gating unchanged.

**Forward-pointer — methodology question (NOT scoped here, NOT a track opened by this PR):** how to validate the quiescence mechanism when Wi-Fi does not reproduce Mode 2 (no symptom) AND Tele2 LTE breaks the REST fallback substrate (no working venue). Three candidate paths for operator decision: (a) find a third network class that reproduces Mode 2 AND keeps REST alive (different LTE carrier, tethered Wi-Fi through mobile hotspot, degraded-RSSI Wi-Fi); (b) synthetic / instrumentation-only triggers (carries false-confidence risk); (c) state-machine / integration-level validation against a fake transport that scripts the Mode 2 surface (carries model-vs-field divergence risk). A separate facts-first recon — working name `QUIESCENCE-VALIDATION-METHODOLOGY-RECON1` — is the natural place to discriminate, but it is NOT opened by this PR; it is a forward-pointer for the operator's next decision.

**Side-findings preserved (NOT actioned here):**

- Operator's QR-add → reverse-add warm-up flow was the same paired-bootstrap pattern as K-1; not a scenario deviation.
- Receiver-side decrypt counts (Tecno 6, emu 10) lower than expected for 4 × 2 = 8 scheduled messages because the analysis script's regex counted only `DECRYPT_TRACE ok` and missed `inbound_repair_ok` events (which carry plaintext bytes too). Future K-runs should add `inbound_repair_ok` to the receiver-side delivery count to avoid under-counting deliveries that took the repair path.

**Follow-ups:** the parent recon stays open with B1 unresolved. The methodology recon opens (if ever) on a separate operator-greenlit mini-lock, NOT auto-opened by this PR. RC PR #330 stays Draft / HOLD; gating unchanged.
...
**Branch series at draft-PR-open time (head `ecd67e04`):** commit 1 (`69c8452c`, `disconnectAndJoin` lifecycle API + critical NonCancellable teardown), commit 2a (`70ace30b`, typed `sealed interface WsReconnectGate` in `WsReconnectGate.kt` + the single-lock atomic gate surface implementation on `RestStateMachine.kt` — owner-validated, generation-floor, bypass-locked), commit 2b (`35af30fb`, `runReconnectLoop` observes the gate), commit 2c (`89505e66`, `TransportRewalkCoordinator` transaction + `lastRewalkAtMs` invariant + typed `RouteChangeOutcome`), commit 2d (`c72c2b98`, `BuildConfig.RECONNECT_QUIESCENCE_ENABLED` + `AppContainer` flag wiring + production `currentKindProvider` / `tokenSource` + release-pin + first-cut Test 20), commit 2e (`4e77f572`, closing test package — full integration `HybridRelayTransportIntegrationTest20` + deterministic flush-cancellation anchor + two strengthened test gaps — squashed from four rounds of chat-driven review iterations that widened seam visibility incorrectly, then reverted to `internal` + reflection bridge in `androidUnitTest`, then added real `bootstrap()`, then added the recovery half, then added a wire-recorder with positive control, then made the teardown fail-loud, then extracted the shared private `runReconnectMergeAndFlush` helper), commit 2e fix-round-4 (`2da35ebe`, removed the `-keep class phantom.core.transport.KtorRelayTransport { *; }` ProGuard wildcard that would have preserved every `*ForTest` seam in the release APK; added the `verifyR8StripsTestSeams` Gradle task wired as `finalizedBy` on `assembleRelease`; fail-loud commonTest teardown on three sites; shared `currentKindProvider` lambda in AppContainer), commit 2e fix-round-5 (`ecd67e04`, widened verifier scope from `phantom.core.transport.*` to all `phantom.*`; bounded the forensic `cleanupInflightForTest` reads inside fail-loud teardown so a stuck mutex cannot re-hang `@AfterTest`; dropped stale references and a duplicate import).

**R8 artifact proof.** Empirical: `assembleRelease` GREEN with the wildcard removed, `verifyR8StripsTestSeams: PASS — no *ForTest members survived on any phantom.* class`. Negative control: temporarily re-adding the wildcard breaks `verifyR8StripsTestSeams` with 127 violations, then removing it again restores GREEN. The mechanism catches future regressions by name.

**Android CI 30-min timeout on first run (`27997609089`).** The `:shared:core:transport:jvmTest` task hung for 28 minutes inside `BodyTimeoutContractTest > r12_body_timeout_after_headers_does_not_retry_immediately` before the GitHub job 30-minute timeout fired. Local sweep on Windows had been GREEN throughout (`:shared:core:transport:jvmTest` 566 tests / 0 failures in ~35 s; `:apps:android:testDebugUnitTest + assembleDebug + compileReleaseKotlinAndroid` 140 tests / 0 failures in ~1 min 35 s). Suspect path: `RestStateMachineTestHelpers.kt:29` (`submitEventNow` wrapping `suspend submitEvent` in `runBlocking { ... }`) called inside `runTest(timeout = 5.minutes)` — `runBlocking` inside `runTest` is a known anti-pattern (test-scheduler dispatcher isolation vs blocking dispatcher). Locally hits the fast path in 0.213 s; on CI Linux x64 hangs. Diagnosis-before-fix: workflow `--max-workers=1` would mask, heap boost would mask, source repair is the root-cause path.

**Plan locked 2026-06-23:** (1) this docs PR — `docs/tracks/rc-reconnect-quiescence1.md` retrospective mini-lock + this `PROJECT_LOG.md` entry — merges first, repairing the rule-3 / rule-6 trail; (2) feature branch rebases onto the new master + two commits in the branch history (`69c8452c`, `c72c2b98`) are reworded via interactive rebase to drop personal attribution per the repo-artifacts-first-person-voice rule; (3) targeted source repair of `submitEventNow` / `onEventNow` call sites that live inside `runTest` bodies (the `BodyTimeoutContractTest` and `RestFallbackOrchestratorBreakerTest` sites visible to grep); (4) force-push the new head; (5) Android CI; (6) on the green SHA, fresh architects review pass (full and adversarial); (7) PR evidence comment with file:line refs + sweep proof + R8 negative test per rule 9; (8) APK with all three flags `"1"` from the exact PR head, SHA-256 recorded; (9) Tele2 LTE field smoke per rule 8 — only then mark PR ready and merge.

**Rule-3 / rule-6 retrospective backfill, honestly framed.** This was a process slip on this track — the feature branch opened and the PR was drafted before `docs/tracks/rc-reconnect-quiescence1.md` landed on master. The docs PR is committed as a separate docs-layer PR per rule 2; the mini-lock file's top section is explicit that it is a retrospective backfill, not a pretense that rule 3 was followed on time. After this docs PR merges the rule-3 contract is in effect for any further work on this track.

**Known limits acknowledged in the draft PR body and in the mini-lock §6:** (L-Test20-invocation-gap) Test 20 drives the shared `runReconnectMergeAndFlush` private helper via the test seam, not the production `runReconnectLoop:1287` call site — a future regression that removes the call site would leave Test 20 green; the structurally correct pin requires an integration test with a recording WebSocket session (Ktor MockEngine WS or embedded loopback server) and is deferred to a follow-up track. (L-Runtime-narrowness) The "no new runtime functionality" framing is too broad — three production-path branches added (`sendRawAttemptForTest?.invoke`, `flushEnteredForTest?.complete`, `flushReleaseForTest?.await`) fire the volatile-field READ on every call; production-observable BEHAVIOR is unchanged when seams are null, which is the rollout-relevant invariant. (L-Workaround-not-root-cause-on-CI) Even if the targeted source repair fixes the BodyTimeoutContractTest hang, any commonTest file that calls `submitEventNow` / `onEventNow` inside `runTest` could re-introduce the same hang; a follow-up audit track would scrub the wider commonTest surface.

**Follow-ups:** docs PR merge → feature-branch rebase + reword → source repair → Android CI green SHA → architects review pass → PR evidence → APK → Tele2 LTE smoke → ready/merge. Rollout PR follows separately and flips `MODE_2_FAST_PATH_ENABLED`, `MODE_2_STICKY_ENABLED`, `RECONNECT_QUIESCENCE_ENABLED` from `"0"` to `"1"` together.
...
- Tecno `<redacted-device-serial>` on Tele2 LTE (Wi-Fi off, mobile data validated against a third-party browser fetch) + emulator `emulator-5554` on host Wi-Fi.
...
**This PR (docs)** — Phase 2 outcome — evidence summary §31-§39 appended to `docs/tracks/rc-direct-ws-death1.md`. **PCAPdroid v12 capture set: 6 sessions × 3 artifacts = 18 files** archived in `C:\temp\phase2-day-2026-06-03\`. Sessions: Arm P3 Wi-Fi control × 1 (APK `09b3ec5c...`), Arm P1 Wi-Fi capture × 2 (APK `2ca6908c...`), Arm P3 Tele2 LTE control × 1 (APK_P3 reinstall), Arm P2 Tele2 LTE capture × 2 (APK `ce8c52de...`). All APKs built on master `358e063e` with `rcDirectArm=B` and one of three `phase2Mode` values per Phase 2 mini-lock §22. Tecno serial `<redacted-device-serial>`. Initial `arm-p1-wifi-cap1-relay.log` was 0 bytes on first pull; re-pull on same field-test day succeeded at 132 940 bytes, promoting Session 2 from PARTIAL to PASS for Mode 1 evidence. Wall-clock alignment per `Inv-WallClockAlignment` §21 verified within ≤ 1 s at field-test day start.
...
Pre-implementation baseline gathered on master `6919b91a` (APK MD5 `d3b7880dc37516ce49330fceda574316` — byte-identical to PR-RECV-DIAG1 Test #86 APK, no source changes since `684c2be6`). Four scenarios on Tecno `<redacted-device-serial>` (real device, Tele2 LTE Иркутская) paired with `emulator-5554` on dev Wi-Fi.
...
**Tests run on real hardware** (Tecno `<redacted-device-serial>` on Tele2 LTE Иркутская + `emulator-5554`):
...
**Test #53 (2026-05-17 evening).** Tecno `<redacted-device-serial>` on
...
**Test #52 setup.** Tecno `<redacted-device-serial>` on Tele2 LTE Иркутская +
...
- **Termux curl from Vladislav's Tele2 LTE handset** (IP `<redacted-client-ip>`, WHOIS = `T2 Russia AS41330` — same network as Test #42, confirmed):
...
- **Unidentified MTS Irkutsk client `1b51ae78…`.** Caddy log shows this identity from IP `<redacted-client-ip>` (PJSC MTS Irkutsk branch, AS8359) doing `GET /auth/challenge` every ~30 s through HTTP/1.1 Direct, stable for hours. Not one of Vladislav's current test devices. Two possible explanations: an old PHANTOM install on a forgotten device of Vladislav's that retained the SQLCipher identity, or a real-world third-party user that found the build somewhere. Filed as low-priority follow-up to confirm via the relay's JSONL prekey records and either ack or quietly investigate further; **no action that affects this user** until we know which it is.
- **Process notes.**
...
- **PR-H1e diagnostic sprint — four runs on `diag/h1e-ws-ping-experiments`.** Three feature flags added in `RelayTransportConfig.kt` so a single APK build could be attributed via a `transport_diag` log line: `EXPERIMENTAL_WS_PING_INTERVAL_MS`, `EXPERIMENTAL_DISABLE_APP_PING`, `EXPERIMENTAL_DISABLE_ALARM_RECONNECT`. `H1E_MARK` timeline markers (reconnect_start / reconnect_success / ws_ping_timeout) added in `KtorRelayTransport.kt` for tcpdump-pcap correlation. Each run = APK install on Tecno phone (`<redacted-device-serial>`) + emulator (`emulator-5554`), tag-filtered logcat capture to `C:\temp\test38-*.log` … `test41-*.log` for ≥12 reconnect cycles per device.
