...
- **#2** (2026-06-26, ~29 min, VPN OFF): NOT TARGET REPRO. No emu / Tecno `connectFailed` to `65.108.154.152:443`. Host probe 1234/1236 SUCCESS, 2 isolated `ec=28`. All Phantom messages delivered (`relay_send_return ok=true`). Direct WSS Mode 2 ping-timeout pattern visible on both devices as a separate signal.
- **#3** (2026-06-27, ~37 min, VPN OFF): NOT TARGET REPRO. No emu / Tecno `connectFailed` to `65.108.154.152:443`. Host probe 1597/1606 SUCCESS, 9 `ec=28` — including a 6-fail host-probe burst cluster at `21:53:10 → 21:54:03Z` (53 s span) without a co-incident emu / Tecno transport event in the same window. UX-visible `prekey_fetch_result=timeout` once (8002 ms, 2 ms over the 8000 ms budget) followed by a 4 s retry succeeding; message delivered, not lost.
...
- **2026-06-27 host-side probe burst cluster (`21:53:10 → 21:54:03Z`, 6 fails / 53 s).** Captured at `C:\temp\smoke-pr333-baseline-i2-v2\host-probe.tsv` on the operator workstation. If a future I-2 reproduction catches a co-incident emu burst PLUS host burst, the standing I-1 hypothesis matrix (H-A REFUTED) would need to be re-examined.
