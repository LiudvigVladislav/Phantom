...
**Follow-up:** After this amendment merges on master, the gate-only carve-out PR opens against §13.1's corrected test-scope wording. It brings `WsReconnectGate.kt` (verbatim or re-authored) + a fresh structural unit-test file (~100-150 LOC) + `RECONNECT_QUIESCENCE_ENABLED="0"` BuildConfig flag + companion release-pin test + minimal inert wiring (if any) + narrowed ProGuard discipline extension (if any). No `RestStateMachine` modifications. The `WsReconnectGateTest.kt` (1172 LOC) and `RestFallbackOrchestratorQuiescenceWiringTest.kt` lift into the MC implementation PR alongside the gate's state-transition implementation on `RestStateMachine`.
