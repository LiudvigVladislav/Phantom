...
### 2026-06-08 · RC-DIRECT-STABILITY1 §14 Arm G v10/v11 wire-level diagnostic — Android libXray multi-segment Reality ClientHello write stall locked; sub-track RC-LIBXRAY-REALITY-WIRE1 scoped (docs-only)

**Goal.** Diagnose why Tecno Android Arm G Wi-Fi sanity v7 never reached `ws_open`, building on 2026-06-07 finding that Reality server + external path are validated for Linux/amd64 clients. Question at session start: with server-side ruled out, what specifically in the Android-side path (libXray gomobile / WebSocket layer / Tecno-stack / carrier) blocks the handshake?

**Outcome.** **Wire-level evidence locked.** Tecno Android libXray Reality outbound write completes one TCP segment (size = current MSS) then stops. Continuation segment containing the rest of the outer Reality ClientHello (~378-380 bytes) is never written, regardless of MSS (tested via server-side iptables -t mangle TCPMSS clamp 1452 → 1200; first segment size changed 1440 → 1188 but count stayed = 1). WSL2 Linux Xray with byte-identical config sends 2 TCP segments (1460+267=1727 bytes) with PSH flags on both, completes Reality handshake to `CopyRawConn splice` + `HTTP/2 200 {"status":"ok"}` from relay. **Council 2026-06-08 amend of mini-lock decision #2** (libXray lifecycle hybrid → persistent-single-instance per Arm G run) applied successfully in v10 code (one Logger started + closing per run, fixed socksPort across sessions) but did NOT fix the underlying multi-write stall. **The Council model "per-session restart causes Bug B" was empirically refuted by the very next experiment.** Sub-track `RC-LIBXRAY-REALITY-WIRE1` scoped (NOT started) with exit criterion "Tecno wire pcap shows multi-segment Reality outer ClientHello arriving at server". Investigation paths in scope (newer libXray ref / smaller uTLS fingerprint / standalone test APK). Out of scope (WS layer / challenge / relay / anything above SOCKS inbound boundary). Time-box 2026-06-15. Council recommended before code work.

**Wording bounds locked (Vladislav-approved, Vladislav-softness).** Locked verdict: "Tecno/Android libXray начинает отправлять outer Reality ClientHello, но после первого TCP-сегмента прекращает передачу. Сервер получает начало TLS-record, видит что данных должно быть больше, ждёт продолжение, не получает его и закрывает соединение FIN без ServerHello." Vladislav-explicit softness: do NOT write "libXray Reality logic не виноват" (we have not pinpointed WHERE inside libXray/gomobile/runtime/native-bridge/write-loop/socket-wrapping/flush/splice the stall occurs); correct framing is "построение начала TLS-record выглядит правдоподобно, но Android/gomobile write path не доводит multi-segment отправку до конца." This is NOT "Reality не работает на Android" (v9 s=1 one-off success against same server). This is NOT "libXray сломан" (record header construct is structurally valid). This is NOT "Arm G PASS"/"Arm G FAIL" (mini-lock discriminator requires reaching ws_open + echo, we are blocked far below WS). Sub-track scope LOCKED, NOT STARTED.

**Evidence chain (decisive in one window 2026-06-08, 03:00-15:30 local):**

1. **v8 wire-level baseline (prior session, 2026-06-08 02:03 local):** 5 sessions all preflight `/health` + challenge `/auth/challenge` timeout at OkHttp `secureConnectStart` phase + callTimeout=10s, `response_code=null` on terminal `callFailed`. `auth_abort_cap_exceeded` fired cleanly at consecutive=5. Hypothesis at end of v8: Reality SOCKS/Reality path delivers no HTTPS to relay.

2. **v9 wide logcat 2026-06-08 03:00 local** introduced `*:V` capture to surface `GoLog` tag (libXray Go-side stdout/stderr forwarded to Android logcat via gomobile binding). s=1 (first session, fresh process) completed full Reality handshake: `dialing TCP to 65.108.154.152:8443` → `tunneling request via 65.108.154.152:8443` → `XtlsFilterTls found tls client hello! 517` (inner TLS to relay) → `XtlsFilterTls found tls 1.3! 1163 TLS_AES_128_GCM_SHA256` → `CopyRawConn splice` → `RC_DIRECT_ARM_G_preflight_result status=200 elapsed_ms=656` → `RC_DIRECT_ARM_G_auth_done auth_elapsed_ms=828`. Then ws_failure timeout at +10s with `response_code=null`. s=2-5 degraded: dial to :8443 succeeded but `XtlsFilterTls found tls client hello` did NOT appear, Reality handshake stalled, `/health` threw timeout. v9 evidence refuted v8 conclusion "Android libXray Reality broken" — Reality DOES work once, then degrades.

3. **Council resolution 2026-06-08** amended mini-lock decision #2 from "libXray lifecycle hybrid: start→Ready→use→stop in session loop" to "persistent-single-instance per Arm G run". Architect 1 + Architect 2 + Vladislav approved. Model: "per-session restart causes Bug B (Reality handshake degradation in s≥2); persistent Xray will fix". Wording bound locked: amends lifecycle only, does NOT claim Arm G PASS, does NOT claim Reality solves Mode 2.

4. **v10 code shipped on `feat/pr-rc-direct-stability1-arm-g-libxray-refresh` branch.** Three edits in `RcDirectArmG.kt`: (a) Xray construction + start + Ready-wait moved OUTSIDE while-loop, stopped ONCE in finally block; (b) WS upgrade uses separate `OkHttpClient` with `ConnectionPool(0, 1, NANOSECONDS)` to discriminate Bug A pool-reuse-over-splice; (c) `RC_DIRECT_ARM_G_ws_about_to_connect` log + WS request tag `"ws_upgrade"` for EventListener phase-attribution. Build successful 38s. APK SHA `6dd83ab2...`.

5. **v10 Wi-Fi sanity FIELD RUN 2026-06-08 11:37 local.** All 5 sessions failed before Reality `CopyRawConn splice`. Persistent Xray lifecycle verified (one `Logger started` at 11:37:01, one `Logger closing` at 11:39:01 in `xray_stop_done s=5 reason=run_complete`; fixed `socksPort=44287` across all 5 sessions). Each session: `preflight_started` → `phase connectStart` → `phase secureConnectStart` → 10s silence → `phase callFailed t=InterruptedIOException msg=timeout` → `preflight_threw elapsed_ms=10010`. Same for challenge. `auth_abort_cap_exceeded` fired at consecutive=5.

6. **v10 server-side tcpdump on VPS during the run** captured 8 connection attempts from Tecno `185.224.99.244` to `65.108.154.152:8443`. Each identical pattern: SYN (mss 1452) → SYN-ACK → ACK → `seq 1:1441 length 1440 Flags [.]` (NO PSH flag) → server ACK 1441 → 10-13 seconds of silence from Tecno → server FIN seq=1 ack=1441 (zero bytes of TLS response from server) → server retransmits FIN 8+ times.

7. **TLS record header parsing of Tecno first segment** showed: `16 03 01 07 1a` = TLS handshake record, TLS 1.0 legacy version, declared record length `0x071a = 1818 bytes`. Tecno actually transmitted 1435 bytes of record body (1440 - 5 header). Missing 383 bytes of the outer Reality ClientHello body that Tecno never sent.

8. **Hypothesis pipeline 2026-06-08 14:30 local — fail2ban / iptables / Hetzner Cloud Firewall.** Server-side checks: fail2ban only `sshd` jail with 4 banned IPs (none = Tecno); iptables INPUT/FORWARD/DOCKER-USER chains clean for Tecno IP and :8443; nftables `tcp dport 8443 counter packets 3 bytes 156 accept` + `dnat to 172.18.0.7:8443 counter packets 2270 bytes 134068` (most :8443 traffic goes through Docker NAT, working); conntrack empty for Tecno IP + :8443 (expected — test was 30 min ago, conntrack TTL ~2 min); Hetzner Cloud Firewall panel showed "No Firewalls have been applied to your server yet"; BLOCKED PORTS only outgoing 25 + 465 (Hetzner mail default). **Server-side ban hypothesis ruled out.**

9. **Bare TCP probe from Tecno** via `adb shell nc -w 5 65.108.154.152 8443 < /dev/null > /dev/null 2>&1; echo RC=$?` returned `RC=0` — TCP connect succeeded from Tecno egress IP `185.224.99.244` to :8443 directly without PHANTOM/libXray involved. **Network-level reachability confirmed.**

10. **WSL2 Linux Xray re-verified PASS in same window 2026-06-08 23:23 local** (felix WSL2 Ubuntu after reinstalling Xray-linux-64.zip from `/tmp` cleared by WSL reboot). Same byte-identical `armg-felix-client.json` config: `dialing TCP to 65.108.154.152:8443` → `tunneling request via 65.108.154.152:8443` → `XtlsFilterTls found tls client hello! 517` → `XtlsFilterTls found tls 1.3! 1163 TLS_AES_128_GCM_SHA256` → `CopyRawConn splice` → curl `HTTP/2 200 {"status":"ok"}` (felix egress IP `93.152.224.216`, changed from prior 185.224.98.146 — ISP IP rotation).

11. **Server-side tcpdump during WSL2 PASS** captured wire bytes: WSL2 sends `seq 1:1461 length 1460 Flags [P.]` (first PSH-segment) followed 61 microseconds later by `seq 1461:1728 length 267 Flags [P.]` (continuation PSH-segment). Total first write: 1727 bytes in 2 TCP segments. Server actively responds with ramping ACKs (2897 → 5793 → 8332 → 11623) and ~2896-byte ServerHello + Certificate segments back to felix. Connection completes normally with FIN.

12. **v11 server-side MSS-clamp diagnostic 2026-06-08 15:00 local.** Iptables -t mangle -I FORWARD ... -p tcp -s 172.18.0.7 --sport 8443 --tcp-flags SYN,RST SYN -j TCPMSS --set-mss 1200 inserted. Counter `1 1252` confirmed rule materially clamped at least one SYN-ACK. Tecno Wi-Fi sanity restarted with `adb force-stop + start`. New tcpdump capture: Tecno first segment changed `1440 → 1188 bytes`. But STILL only ONE segment from Tecno per attempt. Still no continuation. **MTU/MSS black-hole hypothesis decisively ruled out** — segment size changed materially but count stayed = 1. Rule removed cleanly post-test (`-D FORWARD ...` + counter went to 0/0 on -L re-check).

**What this REFUTES decisively** (cross-cutting evidence):

- v8 inference "Android libXray = Linux Go-uTLS therefore equivalent on wire": Tecno gomobile-built libXray and WSL2 Linux Xray produce structurally different wire output for byte-identical config.
- MTU/MSS black-hole: v11 segment count stayed = 1 regardless of size.
- Server-side rate-limit / fail2ban / Hetzner Cloud Firewall: all chains clean for :8443 + Tecno IP, panel confirmed no firewall applied.
- Persistent-vs-per-session Xray lifecycle as proximate cause: v10 persistent Xray shows same single-segment pattern as v9 s≥2 per-session restart.
- Server-side / network-side cause: WSL2 PASS in same window from felix on same external path class.

**What remains OPEN (proximate cause not pinpointed):**

- Exact location within Android libXray stack of the multi-segment write stop. Candidates: gomobile JNI bridge, Go runtime scheduler interaction with native socket, libXray's outbound write loop, Reality outbound's connection-flush sequencing, Android networking stack interaction with Go's syscalls.
- Why v9 s=1 once completed full handshake (including splice + /health 200 via inner TLS) on the same gomobile-built libXray and same config.

**Sub-track RC-LIBXRAY-REALITY-WIRE1 scope (LOCKED, NOT STARTED).** Exit criterion: Tecno wire pcap shows multi-segment Reality outer ClientHello (continuation segment arrives, server returns ServerHello, splice happens). Out of scope: WS layer (Bug A discriminator deferred), challenge, relay, anything above libXray SOCKS inbound boundary. In scope: investigation of libXray AAR / gomobile build configuration; comparison of libXray's `proxy/vless/outbound` write loop to upstream Xray-core's; try newer libXray ref (post-`9a86646da8d8`); smaller uTLS fingerprint experiment (firefox/safari/edge) as diagnostic confirmation NOT production fix; standalone test APK with only libXray + SOCKS + curl /health to discriminate "Arm G harness affects libXray" vs "pure libXray bug independent of harness".

**Next session entry point (FRESH HEAD required).** Read this entry + `project_arm_g_v10v11_libxray_android_write_stall_2026_06_08.md` + amended `project_arm_g_external_validated_2026_06_07.md`. Choose ONE of two investigation paths in sub-track (newer libXray ref vs smaller fingerprint vs standalone APK) — Council recommended before code work. Time-box for sub-track: 1 week from 2026-06-08 = 2026-06-15. If sub-track surfaces "fundamental gomobile limitation that cannot be fixed in PHANTOM scope" → escalate to strategic Council on transport architecture (REST + short-cycle as primary, Reality as opportunistic). NOT before sub-track evidence.

**Diagnostic-design lesson.** The 2026-06-08 Council resolution amending mini-lock decision #2 from hybrid lifecycle to persistent Xray was based on the model "per-session restart causes Bug B (Reality handshake degradation in s≥2)". v10 wire evidence REFUTED this model: persistent Xray applied but the underlying multi-write stall persists across all attempts including s=1. The Council resolution itself was procedurally correct (evidence-driven, not arbitrary), but the model it embodied turned out wrong. **A Council decision can be empirically refuted by the very next experiment.** Wire evidence beats model. Do not lock a sub-track or architectural pivot on a single Council unless the underlying evidence-class is itself wire-confirmed.

**Files shipped this session (docs-only):** MASTER_TIMELINE_2026.md last-updated entry; PROJECT_LOG.md session journal entry (this entry); memory `project_arm_g_v10v11_libxray_android_write_stall_2026_06_08.md` (full evidence chain + Vladislav wording locks); memory amendment to `project_arm_g_external_validated_2026_06_07.md` (Android-equivalence inference REFUTED); MEMORY.md index updated. Zero application code change in this commit (PR-G2 fixup v8/v9/v10 are in prior commits on the same branch as evidence-baseline, NOT promoted to master). Zero relay code change. Zero server-side config change (iptables MSS-clamp rule was temporary, removed cleanly post-v11). xray REALITY production on `:8443` untouched throughout session.

### 2026-06-07 · RC-DIRECT-STABILITY1 §14 Arm G desktop external discriminator — setup validated via Linux Xray; Windows Xray failure classified as diagnostic-tool artifact (docs-only)

**Goal.** Diagnose the cause of Android Arm G v6/v7 0-ws_open evidence (TEST A v6 reinstall = 7 sessions all challenge_threw timeout; v7 = 54 Wi-Fi + 66 Tele2 sessions all challenge_threw timeout, 0 ws_open). Question at session start: is Reality `:8443` external ingress broken for non-loopback clients, or is Android libXray runtime specifically broken?

**Outcome.** Setup validated — Reality server, embedded `OperatorXrayConfig.kt` constants, external network path felix LAN → Hetzner `:8443`, and cover-host anti-probe fallback `dest=www.microsoft.com:443` all confirmed healthy. Windows xray.exe 26.3.27 identified as broken diagnostic oracle (1440-byte ClientHello vs canonical uTLS chrome 517-byte fingerprint — REALITY anti-probe silent-rejects unknown fingerprints with FIN seq=1, wire-indistinguishable from server bug). Six-hour diagnostic chain burned chasing Windows xray.exe artifacts before WSL2 Linux Xray pivot revealed it as a tool bug, not a product bug. Android Arm G v6/v7 prior failures remain OPEN and unexplained — with Reality external path now validated, failure isolates to Android-side (libXray-gomobile runtime / WebSocket layer above SOCKS / Tecno-stack / carrier interaction at higher layer than Reality handshake), not Reality `:8443` server-side.

**Wording bounds locked.** This is NOT "Reality works on Android". This is NOT "Arm G PASS". This is NOT an Arm G outcome. Discriminator (PASS / PARTIAL / FAIL per `project_arm_g_minilock_2026_06_05.md`) still requires Android libXray retest on Tecno Wi-Fi (5-min sanity, look for `auth_done` + `ws_open`) and then Tele2 LTE 15-min v7 spec run if Wi-Fi reaches `ws_open`.

**Evidence chain (decisive in one window 2026-06-07 05:05-06:00 GMT):**

1. **WSL2 Linux Xray 26.3.27 (linux/amd64) on felix**, config `C:\temp\armg-felix-client.json` (byte-identical to embedded `OperatorXrayConfig.kt` values: `serverName=www.microsoft.com`, `publicKey=kDRyYpqpNGT_2IEbJ2pCxpkrinGBhokiNpO4cFOM6w0`, `shortId=ab580a24c7a1e293`, `uuid=09c6fd0e-dc89-4659-a7c3-9c6476590a6a`, `flow=xtls-rprx-vision`), same external egress IP `185.224.98.146` (Ethernet, VPN OFF verified via `Get-NetAdapter` + `curl ifconfig.me`) → `HTTP/2 200 {"status":"ok"}` PASS. Debug log: `XtlsFilterTls found tls client hello! 517` → `XtlsPadding 517 820 0` → `XtlsFilterTls found tls 1.3! 1163 TLS_AES_128_GCM_SHA256` → `Xtls Unpadding new block, content 2539 padding 195 command 0` → `CopyRawConn splice`. Full XTLS-Vision handshake completed.
2. **Windows xray.exe 26.3.27 (windows/amd64) on same felix** in same window, same config file, same external path → `curl: (28) Connection timed out after 15014 milliseconds` FAIL. VPS tcpdump shows: SYN/SYN-ACK/ACK handshake completes, felix sends ClientHello (1440 bytes one segment), server (container 172.18.0.7) ACKs `ack=1441`, then server sends `FIN seq=1 ack=1441` ~8 seconds later — zero bytes of TLS response from server before close. Multiple connection attempts (xray redial loop ports 62274 / 62324 / 62343) all identical pattern.
3. **Loopback A/B fresh on VPS in same window** via `docker run --rm -d --network host -v /tmp/armg-xray-client/config-no-spider.json:/c.json:ro ghcr.io/xtls/xray-core:latest run -c /c.json` → curl through SOCKS5 `127.0.0.1:18080` → `HTTP/2 200 {"status":"ok"}` PASS. Confirms server + REALITY stack + auth keys + cover-host fallback dest all healthy at the moment of the felix tests.
4. **Server config diff via python3** on `/tmp/phantom-xray-config.json` (`docker cp phantom-xray:/etc/xray/config.json` because phantom-xray image is distroless — `docker exec sh` fails with `exec: "sh": executable file not found in $PATH`; jq also unavailable on phantom-relay-01, used python3 inline JSON parse instead). All four DECISIVE CHECKS pass: felix `shortId='ab580a24c7a1e293'` ∈ server `shortIds=['', 'ab580a24c7a1e293']` MATCH ✓; felix `UUID='09c6fd0e-…'` ∈ server `client_ids=['09c6fd0e-…']` MATCH ✓; felix `serverName='www.microsoft.com'` ∈ server `serverNames=['www.microsoft.com']` MATCH ✓; felix `flow='xtls-rprx-vision'` ∈ server `client_flows=['xtls-rprx-vision']` MATCH ✓. Loopback passing client config and failing felix client config are byte-identical at REALITY-settings level. Config-mismatch refuted.

**What VALIDATES:** Reality `:8443` server config, keys, allowed lists, route, anti-probe fallback dest — all working as designed for external Linux clients on real internet path. External network path felix LAN → Hetzner `:8443` clean. Stage 5E.B.5 Reality production endpoint healthy for non-loopback external clients.

**What REFUTES (definitively):** Config mismatch; MTU black-hole on external path; ingress filter / fail2ban / Hetzner Cloud Firewall ban; external path felix→Hetzner broken; server-side REALITY stack broken; server-side cover-host fallback dest broken.

**What CLASSIFIES as broken:** Windows xray.exe 26.3.27 binary as a REALITY client. Generates non-canonical 1440-byte ClientHello that REALITY anti-probe silent-rejects. Rule locked: Windows xray.exe MUST NOT be used as oracle for REALITY in PHANTOM diagnostics. Use WSL2 Linux Xray or Docker Desktop Linux container instead. See `feedback_windows_xray_invalid_oracle_2026_06_07.md`.

**What remains OPEN:** Android Arm G v6/v7 prior failures cause unexplained. Failure isolates to Android-side path (libXray-gomobile runtime quirk / WebSocket layer above SOCKS / Tecno-stack / carrier interaction at higher layer than Reality handshake). Inference "Android libXray = Linux Go-uTLS therefore works like WSL Linux Xray" is NOT proven — libXray gomobile-wrapped may have its own uTLS commit / build path differing from upstream xray-core release. Requires direct Android retest, not inference.

**Diagnostic-design lessons captured (six-hour burn analysis):**

1. **Check your oracle first.** Windows xray.exe ClientHello = 1440 bytes was visible in round 1 of tcpdump; canonical uTLS chrome is 517 bytes. That size delta was a clue actionable at round 1 but only acted on at round 4 (WSL2 pivot). Faster oracle-swap on size mismatch would have saved 2-3 rounds.
2. **Tooling verification before execution.** Assumed `jq` available on VPS (was not), `$PRIVATE_KEY` exported in shell (was not), `sed` regex correct on first try (had `.*\1` instead of `\1`), `docker exec sh` works on distroless container (image has no shell). Each assumption cost ~1 round. Pattern: "assume-then-fail" should be replaced by "verify-then-execute" for one-shot multi-line bash bundles. Verifier list to add to runbook: `which jq`, `command -v sh` (inside container before `docker exec`), `echo "$VAR"` (before using a variable), `grep` test pattern (before piping to sed).
3. **WSL2 / Linux client setup is the cleanest oracle for REALITY truth on Windows desktops.** Architects (architect 1 + 2) both suggested WSL2 two iterations before I escalated to it. Faster pivot would have caught the broken oracle earlier.
4. **distroless container workarounds.** `docker exec phantom-xray sh` fails — image has no shell. Use `docker cp container:/path /tmp/local-copy` then parse on host (python3, jq if available). Locked for future phantom-xray diagnostics.

**Next session entry point.** Tecno Wi-Fi sanity run on current Arm G APK (libXray vendoring ref `9a86646da8d8` / Xray-core 26.3.27 matching server; APK already built on `feat/pr-rc-direct-stability1-arm-g-libxray-refresh` branch through PR-G2 fixup v7). ~5 min, look for `auth_done` + `ws_open`. If Wi-Fi reaches `ws_open` → proceed to Tele2 LTE 15-min v7 spec run with relay DEBUG tail. If Wi-Fi does NOT reach `ws_open` → Reality setup is validated by today's session, so failure isolates to Android-side (gomobile build / WebSocket auth path / Tecno-stack / carrier). Investigation pivots accordingly. Hard deadline 2026-06-12 for §14 closure unchanged.

**Files shipped this session (docs-only):** MASTER_TIMELINE_2026.md last-updated entry; PROJECT_LOG session journal entry (this entry); memory `project_arm_g_external_validated_2026_06_07.md` (full evidence chain); memory `feedback_windows_xray_invalid_oracle_2026_06_07.md` (durable oracle warning); MEMORY.md index updated with two new entries + stale "Next session = PR-G2" pointer replaced. Zero application code change, zero relay code change, zero server-side config change (xray REALITY production on `:8443` untouched throughout session).
