...
On master HEAD the chain from "OkHttp WS death" to "sticky window armed" runs as **two downstream consumers of a single upstream lifecycle event, dispatched SEQUENTIALLY** in `WsSessionLifecycleDispatcher.dispatch(Ended)` — state-machine consumer first, telemetry consumer second. Both consumers matter; the synthetic trigger's injection point determines which consumers it exercises.

**Shared upstream:** `KtorRelayTransport` per-session `finally` block (`shared/core/transport/src/commonMain/kotlin/phantom/core/transport/KtorRelayTransport.kt` lines 1029-1040). Parses the close-error text via `PingTimeoutTextParser.parseSuccessfulPingPongs` to compute `okhttpPingTimeoutDetected: Boolean`. Then `_wsSessionLifecycle.trySend(WsSessionLifecycleEvent.Ended(durationMs, inboundFrames, pendingAcksAtClose, closeOrigin, closeError, okhttpPingTimeoutDetected, sessionEpoch))` — the `trySend` call starts at line 1029; the constructor populates fields 1031-1038. The `_wsSessionLifecycle` is a `Channel<WsSessionLifecycleEvent>` (line 210), exposed as `wsSessionLifecycle: Flow` via `receiveAsFlow()` (line 223).

**Dispatcher (`WsSessionLifecycleDispatcher.dispatch(Ended)`, `apps/android/src/androidMain/kotlin/phantom/android/transport/WsSessionLifecycleDispatcher.kt`):**

1. Lines 54-58: if `restCapabilityActiveProvider()` returns true → `submitStateEvent(event.toRestStateMachineEvent())` (state-machine consumer fires first). Else → `maybeRetryBootstrap()` (bootstrap retry branch when REST capability is off).
2. Line 59: `feedDegradationDetector(event.toLegacyEndedEvent())` (telemetry consumer fires second, ALWAYS — independent of which branch above ran).

The two consumers are sequential, not parallel — the dispatcher is a single coroutine that consumes one event at a time. At L2 (state-machine entry) the trigger invokes the state-machine consumer directly and the telemetry consumer does NOT fire. At L1 (lifecycle channel) both consumers fire in the documented order because the channel-emitted event flows through the dispatcher unmodified.

**Consumer A — state-machine actuation path (R3.6 / 3.6 Fast REST degradation; downstream of dispatcher step 1):**

1. `RestStateMachine.onWsSessionEnded` (`shared/core/transport/src/commonMain/kotlin/phantom/core/transport/RestStateMachine.kt:273`) evaluates the 3-condition signature (lines 295-297): `inboundFrames == 0 && okhttpPingTimeoutDetected && durationMs in MODE_2_MIN_DURATION_MS..MODE_2_MAX_DURATION_MS`.
2. If signature matches AND `mode2FastPathEnabled` is true → emits `mode_2_signature_matched action=fast_path`, calls `transitionToRest("mode_2_fast_path")` (line 307). If additionally `mode2StickyEnabled` is true → `armSticky()` (line 311), which emits `REST_TRACE sticky_armed gen=N reason=mode_2_fast_path` (line 566). Both flags are `Boolean` constructor parameters on `RestStateMachine` (commonMain), populated by Android-side `AppContainer` reading `phantom.android.BuildConfig.MODE_2_FAST_PATH_ENABLED == "1"` / `MODE_2_STICKY_ENABLED == "1"` and passing the Boolean values down (`apps/android/src/androidMain/kotlin/phantom/android/di/AppContainer.kt:1348-1357`). commonMain code never reads `BuildConfig` directly.
3. If signature matches BUT `mode2FastPathEnabled = false` → emits `action=observe_only`, falls through to counter logic.

**Consumer B — telemetry detector path (PR-WS-HEALTH-STATE1 Commit 3.2a; downstream of dispatcher step 2):**

1. `feedDegradationDetectorOnWsSessionEnded` (`WsDegradationCollectorBindings.kt:50`) routes: if `event.okhttpPingTimeoutDetected` → `detector.recordAndEmit(WsDegradationDetector.Event.PingTimeout, currentKind)`. Always: `detector.emitSessionTotal(durationMs, closeOrigin)`.
2. `WsDegradationDetector.recordAndEmit` (`WsDegradationDetector.kt:204`) appends to the sliding window, bumps per-session counter, emits `WS_DEGRADED_TELEMETRY counter` line, then emits `WS_DEGRADED detected` on the rising edge.
3. **Telemetry only on master HEAD.** Per class KDoc (`WsDegradationDetector.kt:59`): "No `markSuspect` action. This detector only logs." The detector takes no actuation on master HEAD.
...
**L1 — Lifecycle event channel (enqueue a synthetic `WsSessionLifecycleEvent.Ended`).** Add a debug-only entry point on `KtorRelayTransport` gated by an injected `debugForceMode2Enabled: Boolean` (or `() -> Boolean` provider) constructor parameter — Android-side `AppContainer` reads `phantom.android.BuildConfig.DEBUG_FORCE_MODE_2_DETECTION == "1"` and passes the Boolean down per the existing `mode2FastPathEnabled` wiring at `AppContainer.kt:1348-1357`. When the injected gate is true, the entry point calls `_wsSessionLifecycle.trySend(WsSessionLifecycleEvent.Ended(durationMs=…, inboundFrames=0, pendingAcksAtClose=…, closeOrigin="synthetic", closeError="DEBUG_FORCE_MODE_2_DETECTION synthetic trigger", okhttpPingTimeoutDetected=true, sessionEpoch=…))`. Same channel the real wire path uses; same downstream consumers (both Consumer A state-machine actuation and Consumer B telemetry detector fire); no production code-path branching.

**L2 — State-machine entry (submit `RestStateMachine.Event.WsSessionEnded` directly).** Debug-only entry point that calls `restStateMachine.submit(Event.WsSessionEnded(…))` with the same 3-condition signature parameters. Bypasses the dispatcher routing entirely; Consumer A (state-machine actuation) runs, Consumer B (telemetry detector) does NOT run.
...
The §5 wording "without becoming a parallel test-only code path that diverges from production" rules out L3 / L4 as honest synthetic triggers and rules out invasive variants of L0. L1 is the cleanest candidate at this design-sketch level; L2 is acceptable for state-machine-only validation but loses Consumer B (telemetry detector) coverage.

### §12.4 L1 design surface (sketch only — NOT a proposal)

The minimum-bypass shape on master HEAD must respect the commonMain / Android isolation rule: shared code (`KtorRelayTransport`, `RestStateMachine`) lives in `commonMain` and cannot see `phantom.android.BuildConfig`. The established master-HEAD pattern for the three existing actuation flags (`MODE_2_FAST_PATH_ENABLED`, `MODE_2_STICKY_ENABLED`, `RECONNECT_QUIESCENCE_ENABLED`) is: Android `AppContainer` reads the `BuildConfig` literal on the Android side and injects a `Boolean` to the shared constructor. A synthetic-trigger flag must follow the same shape.

- **One new Android-side BuildConfig flag** — `DEBUG_FORCE_MODE_2_DETECTION` defaulting to `"0"` in `apps/android/build.gradle.kts` release block. Mirrors the existing flags: release literal `"0"`, debug block `localOrEnv("debugForceMode2", "0")` so operators can opt-in via `-PdebugForceMode2=1`. Companion `Mode2DebugForceReleaseBuildConfigPinTest` in `androidUnitTest` enforces the release literal stays `"0"` (mirrors the existing `Mode2FastPathReleaseBuildConfigPinTest` / `Mode2StickyReleaseBuildConfigPinTest` shape).
- **One new gate-injection point on `KtorRelayTransport`** — a constructor parameter `debugForceMode2Enabled: Boolean = false` (or alternatively `debugForceMode2Provider: () -> Boolean = { false }` if the gate must be re-evaluated lazily — see honesty constraint 4 below). The shared-code class does NOT read `BuildConfig`. AppContainer reads `phantom.android.BuildConfig.DEBUG_FORCE_MODE_2_DETECTION == "1"` (Android side) and passes the resulting Boolean / provider down to `KtorRelayTransport`'s constructor — the same wiring pattern as `mode2FastPathEnabled` at `AppContainer.kt:1348`.
- **One new method on `KtorRelayTransport`** — `internal fun debugForceMode2Synthetic(durationMs: Long): Boolean` (suggested name; `internal` only, no `public` alias). Gated at the top by the injected `debugForceMode2Enabled` Boolean / provider — returns `false` immediately if the gate is off. Refuses to fire (returns `false`) if `_state.value !is TransportState.Connected` so the chain assumption "a session was active" holds; the current session's epoch is read from the live transport. Builds the synthetic event with fields chosen to satisfy the signature check: `inboundFrames = 0`, `okhttpPingTimeoutDetected = true`, `durationMs` validated to lie within `MODE_2_MIN_DURATION_MS..MODE_2_MAX_DURATION_MS` (returns `false` outside range), `closeOrigin = "synthetic"`, `closeError = "DEBUG_FORCE_MODE_2_DETECTION"`, `pendingAcksAtClose = 0`, `sessionEpoch` = current epoch. Calls `_wsSessionLifecycle.trySend(WsSessionLifecycleEvent.Ended(...))` and returns the `trySend` success Boolean.
- **One operator-facing trigger surface** on the Android side — surface up the synthetic-trigger entry to an operator action (e.g., a debug-only menu option, or an `am broadcast` intent handler registered in the AndroidManifest debug block). NOT discussed further at this design-sketch level; depends on operator preference.
...
6. **Field-validation scope honesty.** A synthetic-triggered Mode 2 validates everything from `WsSessionLifecycleEvent.Ended` downstream — including Consumer A state-machine signature check + sticky arming + gate quiescence (PR #330) + recovery probe + 60s probation + sticky clear, AND Consumer B telemetry detector counter/verdict emission. It does NOT validate: the OkHttp ping watchdog firing on a real radio death, the `PingTimeoutTextParser` regex against the actual phrase OkHttp emits, the `KtorRelayTransport` finally-block assembly that builds the production event. The first two have their own unit coverage on master (`PingTimeoutTextParserTest` 13 cells; the watchdog's input semantics are covered by `KtorRelayTransport` integration tests on PR #330's branch but not on master HEAD). The third has no dedicated test but is one block of straight-line code; a structural code-review check that the synthetic populates the SAME fields with the SAME types is sufficient.
...
- **L-13.3.9 — One-shot per `Connected` session epoch + explicit race contract.** Two distinct guarantees, separately scoped:

  - **Part A (repeated-trigger DoS prevention):** the synthetic trigger fires at most once per `Connected` epoch — latch resets on each new `WsSessionLifecycleEvent.Connected` event. Closes the repeated-trigger DoS surface identified by the security council: an attacker who somehow activates the surface cannot keep the gate Quiesced indefinitely by repeated calls.
  - **Part B (check-then-trySend race remains an open design constraint):** the one-shot latch does NOT on its own close the race identified by the implementation council. The underlying `_state.value` can change between the `_state.value is TransportState.Connected` guard and the subsequent `_wsSessionLifecycle.trySend(...)`. A real session death in that gap can produce a real `Ended` for epoch N while the synthetic `Ended` for the same epoch N is also in flight, and the one-shot latch does not prevent that interleaving. The race is recorded here as an explicit design constraint the MB scope-lock MUST resolve. The MB scope-lock MUST specify:
    - the exact epoch snapshot taken at the top of `debugForceMode2Synthetic` so the in-flight synthetic event carries the epoch read at entry, not a stale re-read;
    - the dedup contract on the receiving side — whether the dispatcher / state machine de-duplicates by `(sessionEpoch, closeOrigin)` tuple, or relies on `RestStateMachine`'s existing epoch filter, or another mechanism;
    - the test cells that exercise the race window adversarially (forcing a real session death between guard and trySend) and assert the worst-case behaviour is well-defined.

  This lock does NOT pick the Part B resolution. It pins the requirement that one must be specified and tested before any L1 implementation lands. The MB scope-lock will not be considered complete without a documented answer to Part B.
...
- Pick the operator-surface concrete shape. The mini-lock selects an S6-style debug-only Activity (matching `S6BreakerTriggerActivity` four-layer pattern: debug-overlay manifest only / signature-level `android:permission` / runtime `BuildConfig` re-check / `AppContainer` re-check), OR any non-Activity alternative only if it satisfies the same four layers explicitly. A raw `am broadcast` BroadcastReceiver without all four layers is forbidden per L-13.3.7. The mini-lock states the chosen shape; no future PR can shortcut.
