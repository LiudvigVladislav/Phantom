...
- **Minimal wiring — only if needed.** Wiring stubs (constructor parameters, AppContainer-side wire-up reading the BuildConfig and injecting a Boolean, etc.) are NOT mandatory. They are added ONLY if the carve-out introduces a compile-time dependency that requires them — e.g., if `WsReconnectGate.kt`'s extraction surfaces a new constructor parameter on an existing class that AppContainer constructs. Per §13.5, `WsReconnectGate.kt` as it stands on PR #330 commit `6f49cd89` is a pure type-and-interface declaration file (5 sealed `WsReconnectGate` states, four sealed result/outcome hierarchies, two abstract provider interfaces, `ProbeToken / ProbeBudget` data types, a `simpleKind()` extension) — extracting it imposes NO compile-time dependency on any existing class. The expected default is therefore "no wiring stubs". If the carve-out implementer concludes wiring stubs are needed after all, the carve-out PR body MUST cite the specific compile-time dependency that forced them. Any wiring stubs introduced MUST default to `false` in release and MUST NOT change the runtime behaviour of any existing transport code path when the flag is `false` — same hard rule as the BuildConfig flag itself.
...
- `WsReconnectGate.kt` (PR #330 commit `6f49cd89`, 447 LOC) is, by contrast, a pure type-and-interface declaration file: 5 sealed `WsReconnectGate` states; `ProbeToken` value class with `[REDACTED]` `toString()`; `ProbeBudget` data class with locked constants; the `WsReconnectPermit / ClaimResult / ProbeIssueResult / RouteChangeOutcome` sealed hierarchies; the `WsReconnectGateProvider / RewalkCoordinatorGateProvider` abstract interfaces; a `simpleKind()` extension. The file contains no state-transition, transport-routing, or provider-implementation logic — the only runtime code is type-level (`toString()` redaction overrides on `ProbeToken / ProbeAvailable / ProbeClaimed / ClaimedProbe`, and the `simpleKind()` extension returning a telemetry-safe label). The two provider interfaces are declared but NOT implemented in this file; their implementations live on `RestStateMachine` outside the carve-out's scope.
