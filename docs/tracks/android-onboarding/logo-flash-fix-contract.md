> **SUPERSEDED** — 2026-08-14 — this contract described an intermediate shape (`EnterTransition.None togetherWith ExitTransition.None`) that was replaced by the L4 structural fix: Welcome extracted from `AnimatedContent` via a separate `if (navigationStep != Welcome)` branch (see `OnboardingFlowV2.kt:699`). Retained for provenance; not authoritative for current UI. See `c6-onboarding-baseline-landing-contract.md` §7 documentation allowlist.
