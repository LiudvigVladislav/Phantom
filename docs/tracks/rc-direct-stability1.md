...
- **Concurrent short-POST traffic from same identity SUCCEEDED through the same Caddy + Tele2 LTE path during the same 163-second window.** Three `/prekeys/publish` POSTs (`event="prekey_publish" identity=bab6daa26fab3528 opk_count=40`) and one `/auth/session` POST (`event="rest_session_issued" identity=bab6daa2`) completed normally between the first chunk arrival and the slow-POST abort: `10:35:16.518236Z` (prekey_publish), `10:35:16.541050Z` (rest_session_issued), `10:36:17.862272Z` (prekey_publish), `10:37:20.063788Z` (prekey_publish). The same carrier path, same edge, same client device, same identity carried short request/response cycles to completion while the long-held T2 POST went silent after ~5 KB cumulative payload. This is the byte-threshold-class discriminator firing in its predicted direction.
- **Tecno client-side counters NOT captured in this run.** Logcat at `C:\temp\t2-tecno-tele2.log` (133 lines) returned zero matches for the `T2_SLOW_POST` / `RC_DIRECT` tag families — the capture filter for this run did not include the diagnostic tags. The verdict is unaffected because hard gate 2 makes the relay-side counter primary: `write()` accepting bytes proves only OkHttp queue-accept, not physical egress, and the relay-side `total_received=5120` is dispositive regardless of what Android's `total_sent` counter would show. Future Arm G field tests will include the diagnostic-tag filter in the canonical logcat command to recover this secondary signal.
- **Isolation caveat (honest).** T2 was NOT a sole-connection field test. The relay log shows the diagnostic POST was concurrent with production bootstrap traffic from the same install (prekey publish + auth/session issuance after onboarding). The discriminator still holds — the short-lived bootstrap POSTs succeeded while the long-held T2 POST stalled — but I cannot rule out that the precise abort-byte-count (5120) interacted with the simultaneous short-POST traffic stealing carrier-allocated TCP/PDP context. The verdict is therefore "byte-threshold class confirmed" not "exact threshold 5120 ± ε confirmed."
