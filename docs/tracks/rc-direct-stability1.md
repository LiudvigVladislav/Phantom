...
- **Clean transport isolation, NOT structural bootstrap isolation.** Arm G short-circuits in `PhantomMessagingService.onStartCommand` BEFORE production `transport.connect(...)` (Arm G's branch fires from the same `serviceScope.launch` block that the existing Arm A.2 / Arm D / T2 short-circuits use, after `container.initMessagingFromStorage()` and `service.startReceiving()` have already run — see [PhantomMessagingService.kt:344-393](apps/android/src/androidMain/kotlin/phantom/android/service/PhantomMessagingService.kt#L344-L393)). This means the `MessagingService` is **already started** when Arm G fires; whether it generates production `prekey_publish` / `rest_session_issued` traffic during the Arm G capture is a function of MessagingService internal state at that moment, NOT a structural guarantee of the short-circuit position. The T2 Outcome (§13) showed exactly this confound: relay-side `prekey_publish` events (3) and `rest_session_issued` (1) fired in parallel with the T2 long POST. Arm G inherits the same surface area. The honest framing of the isolation constraint is therefore: (i) Arm G never touches production `transport.connect(...)`, so no `KtorRelayTransport` / `TransportManager` / production WS connects to relay; (ii) PR-G2 hard gate (see "Hard gates" §14 below) AND PR-G3 outcome capture both REQUIRE grep-proof against the Tecno logcat that `PREKEY_TRACE` / `REST_TRACE` / production bootstrap traffic is absent in the Arm G capture window — if present, the field run is annotated with the same isolation caveat T2 carried and the verdict wording bounds tighten accordingly. The signed-challenge identity / `myPubKey` / `signingPubKeyHex` are derived the same way the Service derives them today for Arm A.2 / Arm D / T2 (so the relay-side identity is byte-identical to the WS-arm baselines).
...
6. `Inv-ParallelArmIsolation` enforced at the Service short-circuit at the transport-connect level: when Arm G short-circuit fires, NO production `transport.connect(...)` runs, so no production `KtorRelayTransport` WS connects to relay in parallel with Arm G's WS. **However**, `container.initMessagingFromStorage()` and `service.startReceiving()` have already run before any diagnostic arm short-circuit (see [PhantomMessagingService.kt:344-393](apps/android/src/androidMain/kotlin/phantom/android/service/PhantomMessagingService.kt#L344-L393)), so MessagingService internal state may still generate `prekey_publish` / `rest_session_issued` traffic on its own short-lived REST connections. This is the same surface T2 hit per §13 Outcome isolation caveat. PR-G2 review checklist requires: documentation of this carve-out inline in the diagnostic class file header AND in the Service short-circuit comment block. PR-G3 outcome capture requires: grep-verify the Tecno logcat (`grep -E "PREKEY_TRACE|REST_TRACE|prekey_publish|rest_session"` against the UTF-8-decoded logcat — see §13 T2 Outcome UTF-16 mismatch lesson) and the relay log (`grep -E "event=\"prekey_publish\"|event=\"rest_session_issued\""`) over the Arm G capture window. If hits are present, the outcome subsection records the count and timing; the verdict wording bounds tighten to "Arm G result class confirmed, with concurrent-bootstrap caveat similar to T2." If hits are absent, the outcome wording is unqualified on the isolation dimension.
