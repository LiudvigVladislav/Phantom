...
| #2 | 2026-06-26 | ~39 min | OFF | **NOT TARGET REPRO** | No emu / Tecno `connectFailed` to `65.108.154.152:443`. Host probe: 1581 data rows / 1564 SUCCESS / 17 `ec=28` fails. Two isolated early fails at `18:41:25Z` and `18:41:40Z`, then a dense cluster of 15 fails in ~3 minutes spanning `18:47:36 → 18:50:47Z`. None co-incident with Phantom-side transport events. All Phantom messages delivered (`relay_send_return ok=true`). Direct WSS Mode 2 ping-timeout pattern visible on both devices as a separate signal. **Evidence file:** `C:\temp\smoke-pr333-baseline-i2-v2.1\host-probe.tsv` on the operator workstation, SHA-256 `80153e4f702c1edee2aa8f9413407803b67a6e9858f40589009d1e24d342b12e`, window `2026-06-26T18:16:39.823Z → 2026-06-26T18:55:52.678Z`. |
| #3 | 2026-06-27 | ~39 min | OFF | **NOT TARGET REPRO** | No emu / Tecno `connectFailed` to `65.108.154.152:443`. Host probe: 1672 data rows / 1650 SUCCESS / 22 fails (21 × `ec=28` connect-timeout + 1 × `ec=35` TLS handshake error). Two notable burst clusters: 6 fails in 53 s at `21:53:10 → 21:54:03Z` and 13 fails in ~138 s at `21:56:23 → 21:58:41Z`. None co-incident with emu / Tecno transport events — devices were idle during these windows, so the host blip clusters could not be cross-correlated against any Phantom-side attempt. UX-visible `prekey_fetch_result=timeout` once (8002 ms, 2 ms over 8000 ms budget) followed by a 4 s retry succeeding (`http_bundle_fetch_done status=200` → `relay_send_return ok=true`); message delivered, not lost. **Evidence file:** `C:\temp\smoke-pr333-baseline-i2-v2\host-probe.tsv` on the operator workstation, SHA-256 `15527f291e53b8738b291f92592991a907148a830e48a472307d4d57477fafea`, window `2026-06-26T21:19:24.843Z → 2026-06-26T21:58:53.769Z`. The PowerShell `Import-Csv` analysis script the operator ran mid-loop (`1286 / 1285 / 1`) reflects the file state at evaluation time — the probe loop continued for another ~16 minutes and accrued the bulk of the 22 fails after that snapshot, including both burst clusters. |
...
- **Host-side probe burst clusters on BOTH clean attempts.** Strengthens a side-pattern that none of the previous I-1 / I-2 analysis-rounds singled out. Attempt #2 (`v2.1` file, SHA `80153e4f...`): 17 fails, with 15 clustered in `18:47:36 → 18:50:47Z` (~3 minutes). Attempt #3 (`v2` file, SHA `15527f29...`): 22 fails, with 6 clustered in `21:53:10 → 21:54:03Z` (~53 s) AND a denser 13-fail cluster in `21:56:23 → 21:58:41Z` (~138 s). None of these host-side fail-clusters were co-incident with emu / Tecno transport events — Phantom devices were idle through both windows, so the corpus does NOT support promoting the host-probe pattern to a new hypothesis. If a future I-2 reproduction catches a co-incident emu burst PLUS a host-probe cluster of this shape, the standing I-1 hypothesis matrix (H-A REFUTED, H-B strong) would need to be re-examined — that combined evidence could reopen H-A or H-C in a way the original 2026-06-26 corpus did not allow. Both TSV files are preserved on the operator workstation at the paths and SHAs cited in the §10 attempt table.
