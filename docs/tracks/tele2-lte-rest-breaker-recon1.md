...
- **`totalMs` tightly clustered at the 35 s timeout ceiling — observed range `35002–35006 ms` across all 11 events (total spread 4 ms; distribution: 1 × 35002, 1 × 35004, 5 × 35005, 4 × 35006).** Wall-clock instants: `13:17:33.935, 13:18:18.791, 13:18:26.774, 13:19:09.322, 13:19:14.354, 13:19:50.140, 13:20:38.863, 13:21:34.054, 13:22:51.920, 13:24:50.726, 13:27:26.070Z`.

A timeout firing within a 4 ms band at the same ~35 s ceiling on every occurrence is mechanistically inconsistent with H-OrchestratorCancellation — if the cause were orchestrator cancellation driven by WS state transitions, the elapsed time at cancellation would vary by 0-35 s depending on when the WS event arrived in the call's lifetime. The observed clustering at the budget ceiling indicates each call ran to its own timeout, not to an external interrupt.
...
Neither failure is in the application code at master HEAD. The R3.6 breaker policy is correctly observing the REST failure pattern; the WS deliver-in path is correctly wired and would deliver if the WS sessions became alive. **Evidence strongly points to Tele2 LTE long-held connection body loss as the shared mechanism for both failures**; final on-the-wire confirmation requires M-2 packet capture. This direction of investigation is consistent with the carry-forward "long-connection-uplink finding" from `rc-direct-stability1.md` §13, but the K-2 corpus alone cannot fully exclude unrelated mechanisms (e.g. relay-side response truncation under load, an interaction with HTTP/2 stream backpressure, or a TLS-layer renegotiation race) without packet-level evidence.
