...
- **D15 — Bad-MAC posture (poison-envelope safe response).** The earlier draft of D15 proposed an ack + cursor advance after a bounded retry count. **That draft is REJECTED.** A bad-MAC envelope MUST NOT be acknowledged and MUST NOT advance the persisted cursor at any retry count. If MAC verification drifts because of a stale verify key, a canonical-encoding bug, a client/server version skew, or local DB corruption, an ack-and-advance posture silently and irreversibly destroys real messages — the exact guardrail-A violation `seq_mac` was introduced to detect. `seq_mac` defends against poll-layer / DB tamper / bugs per the locked threat model; the right response to a sustained verification failure is to assume the verifier is wrong about THIS envelope, not to delete the relay's authoritative copy.
  ...
  - **One forced session/key refresh.** On the first repeat (recommended `MAC_REPEAT_REFRESH_THRESHOLD = 2`), force a fresh `acquireOrRefreshToken(reason="poll_mac_repeat", staleToken=token)` so a stale `_seqMacVerifyKey` from a degraded earlier session is replaced and `seqMacVerifyKey` state transitions through `KeyPresent → fresh KeyPresent`. This addresses the most common drift cause without touching the relay's queue.
  - **Suspend / degrade the long-poll path if the same envelope still fails after the refresh.** Cancel `wsActivePollJob` (or transition its `LongPollBreakerState` to a new `LongPollBreakerState.SuspendedOnPoison` named state) and log `event=poll_mac_repeat_suspend reason=verify_fail_after_refresh`. Direct WSS, the legacy `pollLoop`, and other transports stay operational; the messenger remains usable while the bad envelope is investigated.
  - **No ack. No delete. No cursor advance past the envelope.** Under any retry count, on any verifier state.
  
  Resolving queue liveness for a sustained-bad envelope is OUT OF SCOPE for Stage 2B-B. It requires a separate server-side design: server-side MAC revalidation, server-side quarantine on repeat client report, or an authenticated poison-report mechanism. None of those are Stage 2B-B work. The locked posture above explicitly accepts that, under a sustained-bad envelope, the long-poll path becomes the degraded transport while the rest of the messenger keeps running — that is the correct trade-off given the threat model.
  
  Test cells: **M-B12** pins "MAC repeat counter increments but no ack, no cursor advance"; **M-B13** pins "first repeat triggers forced session refresh"; **M-B14** pins "second repeat after refresh transitions to `SuspendedOnPoison`"; **M-B15** pins "Direct WSS path remains operational while `wsActivePollJob` is suspended"; **M-B16** pins "the persisted cursor and the relay's queue remain byte-identical to the pre-poison state across all retry counts." OQ-7 is **rejected** (no threshold approves cursor advance on unverified envelope); the `MAC_REPEAT_REFRESH_THRESHOLD` value is the only quantitative parameter and is locked at 2 unless re-locked.
