...
- **Option A — Stack on any already-landed narrowing commit that provides equivalent ProGuard removal + R8 strip verification.** A qualifying narrowing commit is one that, at the time the implementation PR opens, has already merged to master and (i) removes the `-keep class phantom.core.transport.KtorRelayTransport { *; }` wildcard from `apps/android/proguard-rules.pro` and (ii) adds an R8-strip verification mechanism that fails the release build on any surviving member that should not survive (the existing `verifyR8StripsTestSeams` Gradle task pattern from PR #330's mini-lock §2 is the reference shape, but any equivalent mechanism that satisfies the same defence-in-depth goal qualifies). The qualifying narrowing commit MAY come from PR #330 if PR #330 ships its narrowing portion (e.g., as a carved-out independent PR or as part of its eventual merge), OR it MAY come from a separate independent narrowing-only PR opened specifically to unblock this mini-lock. The implementation PR's commit message + body MUST cite the specific narrowing commit it stacks on (squash SHA + brief description of what the narrowing did). Under Option A, the implementation PR's initial diff does NOT carry ProGuard changes — they are inherited from the base commit.

  Amendment note (2026-06-30): the original §5.1 Option A wording (committed in PR #350 squash `e6a162d9`) literally said "Stack on PR #330's narrowing commit", which created an unintended dependency loop — PR #330 stays Draft / HOLD until B1 closes, B1 closes only after MB PASS lands, MB lands only after the implementation PR opens, the implementation PR opens only under Option A or Option B, Option A required PR #330's narrowing. This amendment loosens Option A to accept any qualifying narrowing commit so a separate narrowing-only PR can unblock the sequence cleanly without touching PR #330. The amendment does NOT weaken the defence-in-depth requirement; the narrowing commit must provide equivalent ProGuard removal + R8 strip verification regardless of source.
...
- **P-1 — The landed-narrowing reference changes.** If the narrowing source the implementation PR plans to stack on (Option A) pivots away from the `verifyR8StripsTestSeams` Gradle task pattern OR equivalent — whether that source is PR #330 or a separate independent narrowing PR — the L-13.3.6 Option A reference here becomes stale. The mini-lock parks until the new shape is documented and Option A's "qualifying narrowing" definition is re-verified against the new mechanism.
