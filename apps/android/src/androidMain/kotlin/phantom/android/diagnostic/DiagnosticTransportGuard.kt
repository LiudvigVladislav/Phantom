...
object DiagnosticTransportGuard {
    ...
    /**
     * Populated by debug boot init with a lambda reading the live
     * `AppContainer.transportManager.state.value`. HRT calls it to
     * observe the actually-selected outer transport arm at send
     * time — this is the §12 P0-3 fix. Reading the live manager
     * state (not the privacy-mode preference the user picked)
     * matters because a Standard-policy device that fell through to
     * Reality or Tor returns those values, NOT `"direct"`; the
     * privacy-mode preference alone would misreport the outer arm
     * in that case.
     *
     * In release the reader is never installed and the field stays
     * null → HRT logs `outer_transport=unknown`.
     *
     * Returns one of `"direct"`, `"reality"`, `"tor"`, `"probing"`,
     * `"failed"`, `"idle"`, or `"unknown"` — never throws.
     */
    @Volatile
