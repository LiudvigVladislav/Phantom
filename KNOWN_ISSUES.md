...
- **Periodic app-level RelayMessage Ping/Pong over WS frames is not sent.** Run C empirically doubled WS lifetime by removing that redundant heartbeat layer; tighter cadences (Run B 5 s) made the cycle worse, not better. The recurring heartbeat remains OkHttp's protocol ping. A quiet REST-backed WSS candidate sends one correlated application-level probe per session, late in probation, solely to prove the return data path. After promotion, a quiet session may send at most one additional proof before the 60-second inbound-stall fallback; an inbound frame rearms that bounded proof for the next quiet window. `APP_LEVEL_PING_ENABLED` remains dead historical config and a future cleanup may delete or rename it.
